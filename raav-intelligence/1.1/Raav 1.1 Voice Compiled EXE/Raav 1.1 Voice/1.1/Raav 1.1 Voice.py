...
# We are not using the internet check in this build
...
def verifyAnswer(questioncheck, checkorfind):
	...
	if memtemp == False:
		...
		if checkorfind == True:
			...
			memoryRestoreAnswer = Memoryx.findvalue(memtemp, answer_memory_value)
...
def mainline(QuestionAsked):
	...
	RecognizeX(TalkToProgram)
#--------------------------Code------------------------------------------

def Recognize(typeortalk):
	if typeortalk == True:
	...
	if typeortalk == False: 
		textToPass = input("Command: ")
		mainline(textToPass)
'''internetonoff = internet_check(300)
if internetonoff == False:
	offlinemode = True
else:
	offlinemode = False
